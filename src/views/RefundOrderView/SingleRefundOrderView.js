...
export const SingleRefundOrderView = (props) => {
  ...
  async function fetchRefundOrder() {
    ...
    try { 
      ...
      const response = await request.get(`/refund-orders/${id}`)
      if(response.code === 200) {
        ...
        setEditable(response.data[0].source === 'web')
      }
    } catch(e) { 
  ...
  useEffect(() => {
    const { isWorking } = props.refundOrder
    if(!isWorking) {
      fetchRefundOrder()
    }
  }, [props.refundOrder])
  ...
  return (
    ...
    >
      ...
      <Row gutter={8} justify={"space-between"}>
        <Col>
          {
            !order.instockStatus && isEditable && (
              <Popconfirm
        ...
        <Col>
          ...
          }
          {
            !order.instockStatus && isEditable && (
              ...
              >
                <Button disabled={isOrderCanceled} type="primary" loading={props.refundOrder.isWorking}>Nhập kho</Button>
              </Popconfirm>
            )
          }
        </Col>
...
const mapDispatchToProps = dispatch => ({
  confirmPaymentStart: (payload) => dispatch(RefundOrderCreators.confirmRefundPaymentStart(payload)),
  createReceiptStart: (payload) => dispatch(RefundOrderCreators.createRefundReceiptStart(payload)),
  cancelOrderStart: (payload) => dispatch(RefundOrderCreators.cancelRefundOrderStart(payload)),
})
