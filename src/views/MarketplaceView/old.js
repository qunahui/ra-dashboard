...
export const CreatePlatformStep = (props) => {
  ...
  const createLazadaProduct = (generalData) => {
    ...
    const skuAttrs = {}
    lazadaAttr.map(i => {
      if(i.attribute_type === 'normal' && i.name !== 'color_family' && i.name !== 'size') {
        attrs[i.name] = i.values
      } else {
        skuAttrs[i.name] = i.values
      }
    })
    let skus = []
  ...
  const renderSystemBrand = () => {
    return <BrandSelect
      ...
      onChange={value => setFormValues({ brand: value })}
    />
  }

  const renderLazadaBrand = () => {
    let newLazAttr = [...lazadaAttr]
    let index = newLazAttr.findIndex(i => i.name === 'brand') 
                ...
                <>
                  ...
                  <Row align="middle" gutter={[16,8]} className={"padding"} style={{ width: '100%'}}> 
                  {
                    lazadaAttr.length > 0 && lazadaAttr.map((attr, index) => {
                      if(attr.is_mandatory && !['name', 'brand', 'SellerSku', 'price', 'package_weight', 'package_length', 'package_width', 'package_height'].includes(attr.name)) {
                        return (
                          <Fragment key={Math.random()}>
