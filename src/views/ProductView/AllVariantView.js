...
//
import { Table, Row, Col, Divider, Tabs, Typography } from 'antd'
import { LinkOutlined, PlusCircleOutlined, DownloadOutlined } from '@ant-design/icons'
...
const columns = [
  ...
  },
  {
    ...
    render: (text, record) => {
      console.log(record)
      return <>
      <Link to={`/app/product/${record.parentId}/variant/${record._id}`}>{record.name}</Link><br/>
      {record.store_id}
    </>
    },
  },
...
export const AllProductTab = (props) => {
  ...
  useEffect(() => {
    props.getProductsStart(filter)
  }, [])

  useEffect(() => {
      const allVariants = props.products.reduce((acc, product) => acc = [...acc, ...product.variants.map(va => ({ ...va, parentId: product._id}))], [])
    ...
    >
      <Col span={24}>
        <Tabs 
          ...
          onChange={(value) => {
            setActiveKey(value)
            props.getProductsStart({ type: value })
          }}
        >
...
const mapDispatchToProps = dispatch => ({
  getProductsStart: (payload) => dispatch(Creators.getProductsStart(payload))
})
