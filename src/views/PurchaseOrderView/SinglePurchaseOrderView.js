...
export const SinglePurchaseOrderView = (props) => {
  ...
  async function fetchPurchaseOrder() {
    ...
    try { 
      ...
      const response = await request.get(`/purchase-orders/${id}`)
  ...
  useEffect(() => {
    const { isWorking } = props.purchaseOrder
    if(!isWorking) {
      fetchPurchaseOrder()
    }
  }, [props.purchaseOrder])
          ...
          <Col span={19}>
            ...
            <div className={"order-info"}>
              ...
              >
                ...
                <InputNumber style={{ width: '100%', marginTop: 8}} value={paidPrice} onChange={(value) => setPaidPrice(value)} min={0} max={order.totalPrice - order.paidPrice} step={1000} formatter={amountFormatter} parser={amountParser}/>
              </Modal>
            </div>
          </Col>
          <Col span={5}>
      ...
      <Row gutter={8} justify={"space-between"}>
        ...
        <Col>
          ...
          }
          {
            !order.instockStatus && (
              <Popconfirm
                title="Xác nhận nhập kho cho đơn hàng này ?"
                onConfirm={handleCreateReceipt}
              >
                <Button disabled={isOrderCanceled} type="primary" loading={props.purchaseOrder.isWorking}>Nhập kho</Button>
              </Popconfirm>
            )
          }
        </Col>
...
const mapDispatchToProps = dispatch => ({
  confirmPaymentStart: (payload) => dispatch(PurchaseOrderCreators.confirmPurchasePaymentStart(payload)),
  createReceiptStart: (payload) => dispatch(PurchaseOrderCreators.createPurchaseReceiptStart(payload)),
  cancelOrderStart: (payload) => dispatch(PurchaseOrderCreators.cancelPurchaseOrderStart(payload)),
})

export default connect(mapStateToProps, mapDispatchToProps)(SinglePurchaseOrderView)
